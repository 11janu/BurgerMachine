...
import sys
from BurgerMachineExceptions import ExceededRemainingChoicesException, InvalidChoiceException, InvalidInputException, InvalidStageException, \
...
# ....Correction Burger machine function and incooperation of the requirements
# UCID: 1234-5678-9012-3456 (as of 19/03/2023)
# This function calculates the total cost of the inprogress_burger array by summing up the costs of each item in the array.
# The calculated cost is then returned from the function and the input() message is used to properly display the value in currency format.
...
class BurgerMachine:
    ...
    def __init__(self, menu):
        ...
        self.currently_selecting = STAGE.MainMenu
        self.total_sales = 0
    ...
    def handle_payment(self, expected, total):
        if self.currently_selecting != STAGE.Checkout:
    ...
    def take_order(self, user_input):
        if self.currently_selecting == STAGE.MainMenu:
            if user_input == "1":
                self.currently_selecting = STAGE.BurgerMenu
            elif user_input == "2":
                self.currently_selecting = STAGE.Checkout
            else:
                raise InvalidInputException
        elif self.currently_selecting == STAGE.BurgerMenu:
            if user_input in self.menu:
                self.inprogress_burger.append(self.menu[user_input])
            elif user_input == "done":
                self.currently_selecting = STAGE.Customization
            else:
                raise InvalidInputException
        elif self.currently_selecting == STAGE.Customization:
            if user_input == "done":
                self.currently_selecting = STAGE.Checkout
            else:
                raise InvalidInputException
        elif self.currently_selecting == STAGE.Checkout:
            try:
                expected = float(user_input)
                total = self.calculate_cost()
                self.handle_payment(expected, total)
            except ValueError:
                raise InvalidInputException
    ...
    def display_input_message(self):
        ...
        print("The total cost of your burger is: ${:.2f}".format(total_cost))
        print("Please enter the expected amount in currency format: $X.XX")
    ...
    def print_current_burger(self):
        ...
        class Burger:
            def __init__(self):
                self.buns_price = 2.50
                self.patty_price = 3.50
                self.toppings_price = {"lettuce": 0.50, "tomato": 0.75, "cheese": 1.00}
                self.total_cost = 0

            def add_buns(self):
                self.total_cost += self.buns_price

            def add_patty(self):
                self.total_cost += self.patty_price

            def add_topping(self, topping):
                if topping in self.toppings_price:
                    self.total_cost += self.toppings_price[topping]
                else:
                    print(f"{topping} is not a valid topping.")
    ...
    def run(self):
        try:
            if self.currently_selecting == STAGE.Bun:
                ...
                my_burger = Burger()
                total_cost = my_burger.ingredients()
                print(total_cost)

                # require total to be entered as currency format
                total = input(f"Your total is {total_cost}, please enter the exact value.\n")
                self.handle_pay(expected, total_cost)
